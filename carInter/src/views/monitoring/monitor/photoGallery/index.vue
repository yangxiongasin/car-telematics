<template>
  <div class="mediaGallery">
    <m-scroll :on-refresh="onRefresh" :on-infinite="onInfinite">
      <div class="realMedia no-photos" v-if="noPhotos">
        <div class="realPhotoTitle flex_center_col">
          <div class="font_7B_1" style="text-align: center" >
            ...
            <p class="pd_top_88">近7天内没有拍摄照片</p>
          </div>
        </div>
      </div>
      <div class="operation" @click="sel_del" v-show="!noPhotos">{{sel_del_btn}}</div>
      <div>
        <div class="flex_cen_bet pg_time font_2D_1">
        </div>
        <div class="pg_time_img flex_wrap_bet" >
          <div class="latelyPhoto_cont_img img_100" v-for="item in imageGallery">
            <m-media-detail :media="item"></m-media-detail>
            <label class="mock_mod flex_end_end" v-if="sel_model_show" :for="'pg' + item.serialCode">
              <input type="checkbox" name="photoSel" :value="item.serialCode"
                     :id="'pg' + item.serialCode" v-model="photoCheckValue" @touchstart="checkVal" class="weui-check">
    ...
    <div class="delete_photo flex_center" v-show="del_photo" @click="subDel">
  ...
  export default {
    ...
    data () {
      return {
        ...
        photoCheckValue: [], // 选中图片的值

        counter: 1, // 默认已经显示出15条数据 count等于一是让从16条开始加载
        pageSize: 12,  // 一次显示多少条
        pageStart: 0, // 开始页数
        pageEnd: 0, // 结束页数
        imageGallery: [], // 下拉更新数据存放数组
        BindEquList: '' // 设备信息
      }
    },
    methods: {
      ...
      // 点击删除icon，如果没有选择图片则提示“请选择要删除的照片”，否则sheet出现
      subDel () {
        if (this.photoCheckValue.length === 0) {
          this.$Toast({
            message: '请选择要删除的照片',
      ...
      checkVal (evt) {
        if (evt.target.checked !== false) {
          ...
          this.photoCheckValue.push(checkval)
        }
      },
      // 确定删除
      sure_del (item, index) {
        ...
        vm.del_photo = false
        vm.sel_model_show = false
        let code = vm.photoCheckValue.join(',')
      ...
      getMedia () {
        ...
        vm.$http.post(this.$service.monitor.getMedia, {
          ...
          if (data && data.data) {
            ...
            if (vm.imageGallery.length === totalCount) {
        ...
        if (this.$el.querySelector('.load-more').innerHTML !== '已全部加载') {
          vm.$http.post(this.$service.monitor.getMedia, {
            ...
            for (let i = 0; i < data.data.mediaList.length; i++) {
              vm.imageGallery.push(data.data.mediaList[i])
            }
            let totalCount = data.data.pagerInfo.totalCount
            if (vm.imageGallery.length === totalCount) {
